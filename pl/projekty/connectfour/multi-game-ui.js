...
const uiCopy = {
    en: {
        lastMoveEmpty: 'Last AI move: -',
        lastMove: (move) => `Last AI move: ${move}`,
        won: 'You won',
        lostBoard: '',
        tie: 'Tie game',
        gameOver: 'GAME OVER - You lost.',
        allCompleted: 'All boards completed. You won.'
    },
    pl: {
        lastMoveEmpty: 'Ostatni ruch AI: -',
        lastMove: (move) => `Ostatni ruch AI: ${move}`,
        won: 'Wygrana',
        lostBoard: '',
        tie: 'Remis',
        gameOver: 'KONIEC GRY - przegrana.',
        allCompleted: 'Wszystkie plansze ukończone. Wygrana.'
    }
};

const copy = uiCopy[document.documentElement.lang === 'pl' ? 'pl' : 'en'];
...
function createBoardUI(boardIndex) {
    ...
    lastMoveElement.textContent = copy.lastMoveEmpty;
...
function updateBoardUI(boardIndex) {
    ...
    if (game.game_over) {
        ...
        if (game.wins === 'o') {
            resultElement.textContent = copy.won;
            resultElement.className = 'board-result win';
        } else if (game.wins === 'x') {
            resultElement.textContent = copy.lostBoard;
            resultElement.className = 'board-result lose';
        } else {
            resultElement.textContent = copy.tie;
    ...
    if (lastMoveElement) {
        lastMoveElement.textContent =
            aiLastMove[boardIndex] !== null
                ? copy.lastMove(aiLastMove[boardIndex])
                : copy.lastMoveEmpty;
    }
...
function updateAllUI() {
    ...
    if (gameOverAll) {
        status.innerHTML = `<div class="game-over-message">${copy.gameOver}</div>`;
    } else if (allGamesCompleted()) {
        status.textContent = copy.allCompleted;
    } else {
        status.textContent = '';
    }
}
